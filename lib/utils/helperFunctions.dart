...
class HelperFunctions {
  ...
  static String getHymCategory(int hymNumber) {
    if ((hymNumber >= 1 && hymNumber <= 69) ||
        (hymNumber >= 696 && hymNumber <= 708)) {
      return "Worship";
  ...
  static Future<Map<String, List<Map<String, dynamic>>>>
    ...
    categories.forEach((category) {
      hymsByCategory[category] = allHyms.where((hym) {
  ...
  static Future<bool> isLocalFile(String fileName) async {
    final hyms = await DBConnect().getLocalHyms();
    print(hyms.toString());
    if (hyms.contains(fileName)) {
      return true;
    } else {
      print("not local");
      return false;
